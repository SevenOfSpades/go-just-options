...
func NewService(opts ...options.Option) (*Service, error) {
	...
	port, err := options.ReadOrDefault[Port](opt, optionServicePort, Port(80))
...
// OptionPort will assign value for Port in options.Options set. There is no need for specifying custom type. This can be an integer.
...
func OptionPort(port Port) options.Option {
	return func(o options.Options) {
		if err := options.Write[Port](o, optionServicePort, port); err != nil {
...
// OptionToken will assign value for Token in options.Options set. There is no need for specifying custom type. This can be a string.
// Options can accept multiple values of the same type as long as provided key is unique.
func OptionToken(token Token) options.Option {
	return func(o options.Options) {
		// it will panic when there is an error during write operation
		options.WriteOrPanic[Token](o, optionServiceToken, token)
	}
}
```